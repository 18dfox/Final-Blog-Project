...
class BlogPost(db.Model):
    ...
    # Creates a reference to the USer object, the "posts" refers to the posts property in the User class
    author = relationship("User", back_populates="posts")
    # create author id. uses users.id instead of user.id because the name of our table is users
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Check for user with provided email (to make sure there isn't already an account with that email)
        ...
        if not existing_user:
            ...
            flash("An account with this email already exists. Log in here!")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("The email is not associated with a user. Please try again!")
            return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to sign in to post a comment")
            return redirect(url_for("login"))
...
@app.route("/contact")
def contact():
    return render_template("contact.html", logged_in = current_user.is_authenticated, current_user=current_user)
...
#added a comment to the end of this file